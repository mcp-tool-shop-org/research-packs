...
这个单仓库中的每个包都通过 [`research-os`](https://github.com/mcp-tool-shop-org/research-os) 冻结，`research-os` 是一个控制研究的系统，它强制执行以下原则：*在获得原始数据之前，不允许进行任何合成。* 每个包都包含其完整的证据基础、对抗性审查历史、审查结果、矛盾解决方案，以及一个加密的冻结凭证，该凭证包含每个规范文件的哈希值。
...
每个包的 `pack/` 目录包含完整的冻结包，包括原始数据、摘录记录、`claims.jsonl`、`claim-reviews.jsonl`、矛盾解决方案、处理结果、审查结果以及 `audits/freeze-receipt.json`。冻结凭证包含每个规范文件的 SHA256 哈希值，可以使用 `scripts/verify-pack.mjs` 进行独立验证。

→ 查看 [`docs/artifact-contract.md`](docs/artifact-contract.md) 以获取完整的准入协议。

### 第三类：方法验证
*面向评估 `research-os` 本身的用户。*

`docs/` 目录和 `catalog.json` 文件是方法评估的入口。`catalog.json` 是一个机器可读的索引，包含所有已准入的包，以及其主题、冻结日期和声明总数。`docs/source-quality-notes.md` 记录了每个研究弧线的结果。`docs/operator-playbook.md` 总结了运行冻结包所获得的运营规范。
...
- `pack/audits/freeze-receipt.json` — 包含 SHA256 哈希值，可进行外部验证
- `synthesis/final-report.md` — 清晰引用，可追溯声明
- `synthesis/decision-brief.md` — 决策摘要
- `pack.manifest.json` — 规范的包元数据
...
`verify-pack.mjs` 会重新读取冻结凭证，并重新计算每个规范文件的 SHA256 哈希值。如果显示为 PASS，则表示该包的内容与冻结时的内容完全相同。

---

## 安全

这个仓库不包含任何凭据、密钥或遥测数据。它是一个静态的存档。请参阅 [SECURITY.md](SECURITY.md) 以获取完整的威胁模型和披露流程。

---

## 相关

- [`research-os`](https://github.com/mcp-tool-shop-org/research-os) — 生成这些包的控制系统
